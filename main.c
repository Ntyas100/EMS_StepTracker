...
#include <string.h>
#include <math.h>
#include "st7789.h"       // display driver for LCD
#include "fonts.h"        // fonts for the display
#include "display.h"           // teammate's display layer
#include "stage_calibration.h"
#include "adc_calibration.h"   // ADC sampling code
...
GPIO_PinState button_last_reading = GPIO_PIN_SET;
...
/* USER CODE BEGIN PFP */
int8_t CheckButtonPressed(void); // forward declaration so main() can see it
/* USER CODE END PFP */
...
int main(void)
{
  ...
  // Tell your ADC code which handle to use
  Calibration_SetADC(&hadc1);

  CalibData *calib = Calibration_GetData();  // get pointer once before the loop

  // Run Calibration Stage on loop till Valid
  do {
...
/* USER CODE BEGIN 4 */
int8_t CheckButtonPressed()
{
  GPIO_PinState current_state = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13);

  if(current_state != button_last_reading){
	  last_debounce_time = HAL_GetTick();
	  button_last_reading = current_state;
  }

  if((HAL_GetTick()- last_debounce_time)> 50){ // Threshold for debounce
	  if(button_state != current_state){
		  button_state = current_state;		   // Update button state if threshold passed

		  if(button_state == GPIO_PIN_RESET){ // If True then button pressed
			  return 1;
		  }
	  }
  }
